-- Prevent disabled tournaments from being joined through SECURITY DEFINER RPCs.
-- The same registration-window and UID/capacity validation remains enforced server-side.

create or replace function public.request_tournament_join(
  p_tournament_id bigint,
  p_free_fire_uids jsonb
)
returns bigint
language plpgsql
security definer
...
begin
  ...
  if now() >= (((v_tournament.tournament_date+v_tournament.scheduled_start_time) at time zone 'Asia/Dhaka')-interval '30 minutes')
    then raise exception 'Registration is closed for this tournament.'; end if;
  ...
  if exists(select 1 from jsonb_array_elements_text(p_free_fire_uids) u(uid) where u.uid !~ '^[0-9]{5,20}$')
    then raise exception 'Each Free Fire UID must contain 5 to 20 digits.'; end if;
  if (select count(distinct u.uid) from jsonb_array_elements_text(p_free_fire_uids) u(uid))<>v_required_uids
    then raise exception 'Free Fire UIDs must be unique.'; end if;

  if exists(select 1 from public.tournament_participants tp where tp.tournament_id=p_tournament_id and tp.user_id=v_user_id and tp.status='JOINED')
    then raise exception 'You have already joined this tournament.'; end if;
  if exists(select 1 from public.tournament_join_requests r where r.tournament_id=p_tournament_id and r.user_id=v_user_id and r.status='PENDING' and r.expires_at>now())
    then raise exception 'You already have a pending join request.'; end if;

  select count(*)::integer into v_joined_count from public.tournament_participants tp
  where tp.tournament_id=p_tournament_id and tp.status='JOINED';
  select count(*)::integer into v_reserved_count from public.tournament_join_requests r
  where r.tournament_id=p_tournament_id and r.status='PENDING' and r.expires_at>now();
  ...
  insert into public.tournament_join_requests(tournament_id,user_id,mode,free_fire_uids)
  values(p_tournament_id,v_user_id,v_tournament.mode,p_free_fire_uids)
...
begin
  ...
  if exists(select 1 from public.tournament_participants tp where tp.tournament_id=v_tournament.id and tp.user_id=v_user_id and tp.status='JOINED')
    then raise exception 'You have already joined this tournament.'; end if;

  v_team_size:=jsonb_array_length(v_request.free_fire_uids);
  select count(*)::integer into v_joined_players from public.tournament_participants tp
  where tp.tournament_id=v_tournament.id and tp.status='JOINED';
  ...
  insert into public.tournament_participants(
    tournament_id,user_id,captain_free_fire_uid,free_fire_uids,
    entry_fee,first_prize,second_prize,third_prize,kill_reward
  )
  values(
    v_tournament.id,v_user_id,v_request.free_fire_uids->>0,v_request.free_fire_uids,
    v_tournament.entry_fee,v_tournament.first_prize,v_tournament.second_prize,
    v_tournament.third_prize,v_tournament.kill_reward
  )
  returning id into v_participant_id;

  update public.tournament_join_requests set status='CONFIRMED',reviewed_at=now(),updated_at=now()
